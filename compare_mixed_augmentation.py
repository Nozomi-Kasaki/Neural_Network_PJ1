...
AUGMENTATIONS = ['none', 'mixed']
MIXED_KEEP_PROB = 0.60
MIXED_ROTATE_PROB = 0.15
MIXED_TRANSLATE_PROB = 0.15
MIXED_SCALE_PROB = 0.10
ROTATION_DEGREES = 7.0
TRANSLATION_PIXELS = 1.5
SCALE_RANGE = (0.95, 1.05)

FIG_SAVE_DIR = r'.\mixed_augmentation_figs'
MODEL_SAVE_DIR = r'.\mixed_augmentation_models'
SHOW_PLOT = True
...
AUGMENTATION_STYLES = {
    ...
    'mixed': {'color': '#F58518', 'linestyle': '--', 'linewidth': 1.2},
}
...
def affine_transform(images, kind, rng):
    batch_size, height, width = images.shape
    yy, xx = np.meshgrid(
...
def mixed_augment_images(images, rng):
    output = images.copy()
    choices = rng.rand(images.shape[0])

    rotate_end = MIXED_KEEP_PROB + MIXED_ROTATE_PROB
    translate_end = rotate_end + MIXED_TRANSLATE_PROB
    scale_end = translate_end + MIXED_SCALE_PROB

    masks = {
        'rotation': (choices >= MIXED_KEEP_PROB) & (choices < rotate_end),
        'translation': (choices >= rotate_end) & (choices < translate_end),
        'scaling': (choices >= translate_end) & (choices < scale_end),
    }

    for kind, mask in masks.items():
        if np.any(mask):
            output[mask] = affine_transform(images[mask], kind, rng)
...
def prepare_train_batch(batch_flat, model_type, rows, cols, augmentation, rng):
    ...
    if augmentation == 'mixed':
        batch_images = mixed_augment_images(batch_images, rng)
...
def plot_curves(model_type, histories):
    ...
    fig.suptitle(f'{model_type} Mixed Augmentation Comparison ({COMMON_NUM_EPOCHS} epochs)')
    fig.tight_layout()
    return fig
...
def main():
    ...
    fig = plot_curves(MODEL_TYPE, histories)
    os.makedirs(FIG_SAVE_DIR, exist_ok=True)
    fig_path = os.path.join(
        FIG_SAVE_DIR,
        f'{MODEL_TYPE.lower()}_mixed_augmentation_comparison_{COMMON_NUM_EPOCHS}epochs.png'
    )
